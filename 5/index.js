...
const init = (() => {
    ...
    const boardItem = '<input type="button" class="tic-tac-toe-">';
...
const reloadGame = (symbol) => {
    isEnded = true;
    symbol == "x" ? () => (alert("X won!"), window.location.reload()) : (alert("O won!"), window.location.reload());
}

const winnerCheck = () => {
    boardValues = document.getElementsByClassName('tic-tac-toe');
